        ...
        <div class="ml-10 mt-10 mr-10">
            <div class="grid grid-cols-2 gap-4 mb-10 ">
                <div class="col-span-1">
                    ...
                    <div v-if="isConnected" class="fi-section rounded-2xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 px-3 py-4 remote-participants-list">
                        <header class="fi-section-header flex items-center justify-between gap-x-3 overflow-hidden mb-2">
                        ...
                        <div class="fi-section-content-ctn border-t border-gray-200 dark:border-white/10 mb-3  pt-2">
                            <table style="width:100%">
                                <tr v-for="participant in sortedParticipants" :key="participant.sid" :id="`plist-${participant.sid}`">

                                    <td class="uname">
                                        <button
                                            @click="toggleStaging(participant.sid)"
                                            :class="[participant.isStaged ? 'font-bold' : 'font-medium', 'text-blue-600 dark:text-blue-500 underline hover:underline']">
                                            {{ participant.identity }}
                                        </button>

                                        <!--                                   <a class="font-medium text-blue-600 dark:text-blue-500 underline hover:underline"> {{ participant.identity }}</a>--></td>
                                    <td>
                                        <span v-if="participant.isMuted" v-html="getIconWithColor(baseMutedIcon,'#ff0000')" @click="toggleParticipantMute(participant.sid)"
                                              style="cursor:pointer;"></span>
                                        <!--
                                                                            <span v-if="!participant.isMuted" v-html="getIconWithColor(baseMuteIcon,'#3CB371')" @click="toggleParticipantMute(participant.sid)" style="cursor:pointer;"></span>-->

                                    </td>
                                    <td>

                                        <span @click="toggleStaging(participant)" v-html="getIconWithColor(icoStagedBase, participant.isStaged ? '#3CB371' : '#a1a1a1')"></span>
                    ...
                    <!--
                     <div class="remote-participants-list">
                                            <ul>
                                                <li v-for="participant in sortedParticipants" :key="participant.sid" :id="`plist-${participant.sid}`">
                                                    <span class="uname">{{ participant.identity }}</span>,
                                                    ...
                                                    <span v-if="participant.isHandRaised" v-html="getIconWithColor(icoHandRaised,'#000000')"></span>
                                                    &lt;!&ndash; Weitere Informationen oder Icons &ndash;&gt;
            ...
            <div class="grid grid-cols-4">
                ...
                <div v-if="isConnected">
                    <!-- Button zum Selbst-Staging -->
                    <div v-if="isConnected">
                        <!-- Button zum Selbst-Staging -->
                        <button @click="toggleStaging(localParticipantSid)">{{ this.isStaged ? 'Bühne verlassen' : 'Auf die Bühne gehen' }}</button>
                    </div>
                </div>
                <div v-if="isConnected">
            ...
            <div class="video-area" id="video-area">
                ...
                <section style="width:1200px !important" x-data="{isCollapsed: false,}" class="fi-section rounded-2xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10"
                         id="data.customer-details">
                    <header class="fi-section-header flex items-center justify-between gap-x-3 overflow-hidden px-3 py-4">
                        <div class="grid flex-1 gap-y-1">
                            <h3 class="fi-section-header-heading text-base font-semibold leading-6 text-gray-950 dark:text-white">
                                Discussion Board
                            </h3>
                        </div>
                        <button id="fullscreenButton" type="button" class="flex items-center justify-end">
                    ...
                    <div class="fi-section-content-ctn border-t border-gray-200 dark:border-white/10">
                        ...
                        <div id="stage-area" class="stage-container fi-section-content-ctn">
                ...
                <section x-data="{isCollapsed: false,}" class="my-8 fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10" id="data.customer-details">
                    <header class="fi-section-header flex items-center gap-x-3 overflow-hidden px-3 py-4">
                        <div class="grid flex-1 gap-y-1">
                            <h3 class="fi-section-header-heading text-base font-semibold leading-6 text-gray-950 dark:text-white">
                                Alle Teilnehmer
                            </h3>
                    ...
                    <div class="fi-section-content-ctn border-t border-gray-200 dark:border-white/10">
                        ...
                        <div id="remote-videos" class="stage-container fi-section-content-ctn">
                            <div v-for="participant in remoteParticipants"
                                 v-if="participant && !participant.isStaged"
                                 :key="participant.sid"
                ...
                <!--
                                ...
                                <section x-data="{isCollapsed: false,}" class="fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10" id="data.customer-details">
                                    ...
                                    <div class="fi-section-content-ctn border-t border-gray-200 dark:border-white/10">
                                        ...
                                        <div id="remote-videos" class="video-container">
                                            <div v-for="participant in remoteParticipants"
                                                 ...
                                                 :key="participant.sid"
                                                 class="participant-container">
                                                &lt;!&ndash; Hier wird das Video oder der Track des Teilnehmers angehängt &ndash;&gt;
                                            </div>
                                        </div>
...
#local-stage-container .video-container {
    width: 100% !important;
...
.stage-participant-container {
    flex: 0 0 calc(400px - 10px); /* Doppelt so breit wie die Videos in remote-videos, minus der Gap */
...
let baseMuteIcon = `<svg xmlns="http://www.w3.org/2000/svg" height="26" viewBox="0 -960 960 960" width="26"><path fill="%FILL_COLOR%" d="M480-400q-50 0-85-35t-35-85v-240q0-50 35-85t85-35q50 0 85 35t35 85v240q0 50-35 85t-85 35Zm0-240Zm-40 520v-123q-104-14-172-93t-68-184h80q0 83 58.5 141.5T480-320q83 0 141.5-58.5T680-520h80q0 105-68 184t-172 93v123h-80Zm40-360q17 0 28.5-11.5T520-520v-240q0-17-11.5-28.5T480-800q-17 0-28.5 11.5T440-760v240q0 17 11.5 28.5T480-480Z"/></svg>`;
...
//https://thenounproject.com/browse/icons/term/stage/
...
export default {
    data() {
        return {
            ...
            baseMuteIcon: baseMuteIcon,
            icoStagedBase: icoStagedBase,
            isLocalUserStaged: false,
    ...
    methods: {
        ...
        addTrackToContainer(track, container) {
            if (track.kind === 'video' || track.kind === 'audio') {
                ...
                // Überprüfen, ob der Track ein Participant-Objekt hat und ob der Audio-Track deaktiviert ist
                if (track.kind === 'audio' && track.participant && !track.isEnabled) {
        ...
        async joinRoom() {
            try {
                ...
                localTracks.forEach(track => {
                    if (track.kind === 'video') {
                        const videoElement = track.attach(); // Erstellt und gibt das <video> Element zurück
                        videoElement.id = 'local-video-element'; // Weist dem <video> Element eine ID zu
                        localVideoContainer.appendChild(videoElement); // Hängt das <video> Element an den Container an
                    }
                });
        ...
        onParticipantConnected(participant) {
            ...
            this.fetchStagingStatuses(this.roomSid);
        },
        ...
        setHandStatuses(statuses) {
            ...
            this.remoteParticipants = this.remoteParticipants.map(participant => {
                return {
                    ...participant,
                    isHandRaised: statuses[participant.sid] || false
                };
            });
        ...
        async fetchStagingStatuses(roomId) {
            try {
                const response = await axios.get(`/room/${roomId}/staged-statuses`);
                const stagedStatuses = response.data;

                // Iteriere über die Schlüssel-Werte-Paare des Objekts
                Object.keys(stagedStatuses).forEach(participantSid => {
                    const isStaged = stagedStatuses[participantSid];

                    if (isStaged) {
                        // Stelle den Teilnehmer auf die Bühne
                        this.setParticipantOnStage(participantSid);
                    } else {
                        // Entferne den Teilnehmer von der Bühne
                        this.removeParticipantFromStage(participantSid);
                    }
                });
            } catch (error) {
                console.error('Error fetching staged statuses:', error);
            }
        },


        // Setzt einen Teilnehmer auf die Bühne
        async setParticipantOnStage(participantSid) {
            const participantIndex = this.remoteParticipants.findIndex(p => p.sid === participantSid);

            if (participantIndex !== -1) {
                const participant = this.remoteParticipants[participantIndex];

                // Handstatus zurücksetzen, wenn gehoben
                if (participant.isHandRaised) {
                    participant.isHandRaised = false;
                    // Führen Sie zusätzliche Logik aus, um den Handstatus in der UI zu aktualisieren
                    this.remoteHandDown(participantSid);
                }

                // Unmute, falls der Benutzer stummgeschaltet ist
                if (participant.isMuted) {
                    this.toggleParticipantMute(participantSid);
                }

                // Setzen Sie den Teilnehmer auf die Bühne, wenn er noch nicht gestaged ist
                if (!participant.isStaged) {
                    participant.isStaged = true;
                    this.moveParticipantVideo(participantSid, true);
                }
            } else if (participantSid === this.localParticipantSid) {

                // Logik für lokalen Benutzer

                this.moveLocalVideoToStage(true);


                // Entferne das Hand-Icon, wenn es existiert
                const handIcon = stageContainer.querySelector('.local-raised-icon');
                if (handIcon) {
                    handIcon.remove();
                }
            }
        ...
        /* async setParticipantOnStage(participantSid) {
             if (participantSid === this.localParticipantSid) {
                 // Logik für lokalen Benutzer
                 this.moveLocalVideoToStage(true);
             } else {
                 const participantIndex = this.remoteParticipants.findIndex(p => p.sid === participantSid);
                 if (participantIndex !== -1 && !this.remoteParticipants[participantIndex].isStaged) {
                     this.remoteParticipants[participantIndex].isStaged = true;
        ...
        /*
                async setParticipantOnStage(participantSid) {
                    const participantIndex = this.remoteParticipants.findIndex(p => p.sid === participantSid);
                    if (participantIndex !== -1) {
                        // Setzen Sie den Hand Raised-Status auf false
                        this.remoteParticipants[participantIndex].isHandRaised = false;

                        // Deaktivieren Sie den Mute-Status, wenn der Benutzer stummgeschaltet ist
                        if (this.remoteParticipants[participantIndex].isMuted) {
                            this.toggleParticipantMute(participantSid); // Angenommen, diese Methode schaltet den Mute-Status um
                        }

                        // Setzen Sie den Teilnehmer auf die Bühne
                        this.remoteParticipants[participantIndex].isStaged = true;
                        this.moveParticipantVideo(participantSid, true);
                        this.remoteHandDown(participantSid);
...
// Entfernt einen Teilnehmer von der Bühne
        async removeParticipantFromStage(participantSid) {
            if (participantSid === this.localParticipantSid) {
                // Logik für lokalen Benutzer
                this.moveLocalVideoToStage(false);
            } else {
                const participantIndex = this.remoteParticipants.findIndex(p => p.sid === participantSid);
                if (participantIndex !== -1 && this.remoteParticipants[participantIndex].isStaged) {
                    this.remoteParticipants[participantIndex].isStaged = false;
                    this.moveParticipantVideo(participantSid, this.remoteParticipants[participantIndex].isStaged);
                }
            }
        },

// Eine neue Funktion, um das lokale Video zur Bühne zu verschieben
        moveLocalVideoToStage(isStaged) {

            // const localVideoContainer = document.getElementById('local-video');
            const localVideoContainer = document.getElementById('local-video-section');
            const raisedIcon = localVideoContainer.querySelector('.local-raised-icon');
            if (raisedIcon) {
                raisedIcon.remove();
            }
            ...
            let stageContainer = document.getElementById('local-stage-container');

            if (isStaged) {
                if (!stageContainer) {
                    // Erstelle einen neuen Container, falls noch nicht vorhanden
                    stageContainer = document.createElement('div');
                    stageContainer.id = 'local-stage-container';
                    stageContainer.classList.add('stage-participant-container');
                    stageArea.appendChild(stageContainer);
                }

                // Verschiebe den Inhalt von #local-video in den Container auf der Bühne
                while (localVideoContainer.firstChild) {
                    stageContainer.appendChild(localVideoContainer.firstChild);
                }
            } else {
                if (stageContainer) {
                    // Verschiebe den Inhalt zurück in den ursprünglichen Container
                    while (stageContainer.firstChild) {
                        localVideoContainer.appendChild(stageContainer.firstChild);
                    }
                    // Entferne den temporären Container von der Bühne
        ...
        moveParticipantVideo(participantSid, isStaged) {
            // Dies nimmt an, dass die Funktion nur für entfernte Teilnehmer verwendet wird
            ...
            if (participantElement && targetContainer) {
                // Füge oder entferne Klassen basierend auf dem isStaged-Status
                if (isStaged) {
                    participantElement.classList.add('stage-participant-container');
                    participantElement.classList.remove('participant-container');
                } else {
                    participantElement.classList.add('participant-container');
                    participantElement.classList.remove('stage-participant-container');
                }

                // Verschiebe das Element in den Zielcontainer
                targetContainer.appendChild(participantElement);
            }
        },
        selfStage() {
            this.setParticipantOnStage(this.localParticipantSid);
        },
        // Toggle-Funktion, die die neuen Funktionen verwendet
        /* async toggleStaging(participantSid) {
             const participantIndex = this.remoteParticipants.findIndex(p => p.sid === participantSid);
             if (participantIndex !== -1) {
                 if (this.remoteParticipants[participantIndex].isStaged) {
                     await this.removeParticipantFromStage(participantSid);

                     await this.triggerStagingEvent(participantSid, false); // Trigger Event
                     const response = await axios.post('/cleanup-staging', {
                         participant: participantSid,
                         roomId: this.roomSid,
                         keyPreFx: 'staged',
                     });
                     console.log('toggleStaging asumed->not staged : ' + this.remoteParticipants[participantIndex].isStaged);

                 } else {
                     await this.setParticipantOnStage(participantSid);
                     console.log('toggleStaging asumed->staged : ' + this.remoteParticipants[participantIndex].isStaged);
                     await this.triggerStagingEvent(participantSid, true); // Trigger Event
                 }
             }
         },*/
        ...
        async toggleStaging(participantSid) {
            ...
            if (participantSid === this.localParticipantSid) {
                // Direkter Zugriff auf das berechnete Attribut isSelfStaged
                const isSelfStaged = this.isStaged;


                // Toggle-Logik für lokalen Benutzer
                if (isSelfStaged) {
                    // Lokalen Benutzer von der Bühne nehmen
                    this.moveLocalVideoToStage(false);
                    // Event senden, um zu signalisieren, dass der Benutzer von der Bühne genommen wurde
                    this.triggerStagingEvent(this.localParticipantSid, false);
                    ...
                    this.isStaged = false;
                    //this.isLocalUserStaged = false;
                } else {
                    // Lokalen Benutzer auf die Bühne stellen
                    this.moveLocalVideoToStage(true);
                    // Event senden, um zu signalisieren, dass der Benutzer auf die Bühne gestellt wurde
                    this.triggerStagingEvent(participantSid, true);
                    //  this.isLocalUserStaged = true;
                    this.isStaged = true;
        ...
        /* async toggleStaging(participantSid) {
             if (participantSid === this.localParticipantSid) {
                 // Prüfen, ob der lokale Benutzer bereits auf der Bühne ist
                 const isSelfStaged = this.isSelfStaged;
                 ...
                 if (isSelfStaged) {
                     ...
                     alert('staged');
                     //this.moveLocalVideoToStage(false);
                     this.removeParticipantFromStage(this.localParticipantSid);
                     // Event senden, um zu signalisieren, dass der Benutzer von der Bühne genommen wurde
                     this.triggerStagingEvent(participantSid, false);
                 } else {
                     // Lokalen Benutzer auf die Bühne stellen
                     this.moveLocalVideoToStage(true);
                     // Event senden, um zu signalisieren, dass der Benutzer auf die Bühne gestellt wurde
        ...
        async triggerStagingEvent(participantSid, isStaged) {
            try {
                // Annahme: Sie haben einen Endpoint, der das Event behandelt
                await axios.post('/trigger-staging', {
                    participant: participantSid,
                    isStaged: isStaged,
                    roomId: this.roomSid
                });
                console.log('room:', this.roomSid);
                console.log('Staging Event ausgelöst:', participantSid, isStaged);
        ...
        updateStagingStatus(participantSid, isStaged) {
            const participantIndex = this.remoteParticipants.findIndex(p => p.sid === participantSid);
            if (participantIndex !== -1) {
                this.remoteParticipants[participantIndex].isStaged = isStaged;
                this.moveParticipantVideo(participantSid, isStaged);
            }
        },
        ...
        sortParticipants(participants) {
            return participants.sort((a, b) => {
                ...
                // Standardwert für 'name', falls nicht definiert
                const nameA = a.name || '';
                const nameB = b.name || '';
    ...
    mounted() {
        ...
        this.$nextTick(() => {
            ...
            window.Echo.channel('staging-channel')
                .listen('.staging-event', (eventData) => {

                    console.error('Received staging-channel:', eventData);

                    for (const participantSid in eventData.sid) {
                        if (eventData.sid.hasOwnProperty(participantSid)) {
                            const value = eventData.sid[participantSid];

                            console.log('staged ' + value + ': ' + participantSid);
        ...
        if (button) {
            button.addEventListener('click', () => {
                ...
                if (stageArea.requestFullscreen) {
                    stageArea.requestFullscreen();
                } else if (stageArea.mozRequestFullScreen) { // Firefox
                    stageArea.mozRequestFullScreen();
                } else if (stageArea.webkitRequestFullscreen) { // Chrome, Safari & Opera
                    stageArea.webkitRequestFullscreen();
                } else if (stageArea.msRequestFullscreen) { // IE/Edge
                    stageArea.msRequestFullscreen();
                }
            });
        }
    ...
    computed: {
        sortedParticipants() {
            return this.sortParticipants(this.remoteParticipants);
        },
    }
