...
class VideoRoomsController extends Controller
{
    ...
    public function triggerStagingEvent(Request $request)
    {
        ...
        return response()->json(['message' => 'Staging Event ausgelöst, room_id:'.$room_id]);
    }
    ...
    public function cleanToggleCache(Request $request)
    {
       ...
       event(new StagingToggleEvent ([$participant_id=>'false']));

        return response()->json(['message' => 'cleanToggleCache Event ausgelöst']);
    }
        ...
        function testCache()
        {
            ...
            $_room_id = 'RM757c504cfc3eda74d91dfbce6373e6f4';
            $_participant_id = 'PA6659108d4fa680abab7f0e97f615a326';
            //Cache::forget('staged_' . $_room_id);

            echo $_room_id;

            echo "<pre>";
            print_r(Cache::get('staged_' . $_room_id));
            echo "</pre>";
...
*/
    ...
    public function getStagedStatuses($roomId)
    {
        $staged = Cache::get('staged_' . $roomId, []);
